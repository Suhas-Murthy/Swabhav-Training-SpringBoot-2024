...
public class AccountServiceImpl implements AccountService {
	...
	public Page<AccountDto> getAllAccounts(int pageNumber, int pageSize) {
		logger.info("Fetching all accounts. Page number: {}, Page size: {}", pageNumber, pageSize);
		Pageable pageable = PageRequest.of(pageNumber, pageSize);
		Page<Account> accountPage = accountRepo.findAll(pageable);
		logger.info("Accounts retrieved: {} on page {}", accountPage.getNumberOfElements(), pageNumber);
	...
	public AccountDto addAccount(AccountDto accountDto) {
		...
		logger.info("Generated a new unique Account Number {}",accountNumber);
		...
		logger.info("Sent email to customer with {} after account creation with accountNumber has {}", customer.getCustomerId(), accountNumber);
	...
	private void sendAccountCreationEmail(Customer customer, long accountNumber) {
		...
		String body = String.format("""
				...
				\n
				        Best regards,
				        AurionSwabhavBank
				        """, customer.getCustomerFirstName(), customer.getCustomerLastName(), accountNumber % 10000,
				accountNumber); 
